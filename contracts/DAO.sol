...
contract DAO {
    ...
    // TODO
    // in testing make proposal with not enough votes and speed up time to see if it passes or not
    ...
    // TODO add require statement to prove that it is an investor and it has enough shares
    function withdraw(uint256 amount, address payable to) external {
        _transferEther(amount, to);
    }

    // TODO remove onlyAdmin cause it is and internal func
    function _transferEther(uint256 amount, address payable to)
        internal
        onlyAdmin
    {
        require(amount <= availableFunds, "Not enough funds");
        // TODO change ether transfer to latest way
        to.transfer(amount);
        availableFunds -= amount;
    }
